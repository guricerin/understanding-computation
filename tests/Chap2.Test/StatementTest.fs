...
let ``reduce if`` =
    test "reduce if" {
        let stmt = If(Variable "x", Assign("y", Number 1), Assign("y", Number 2))
        Expect.equal "<<if (x) { y = 1 } else { y = 2 }>>" (Stmt.inspect stmt) ""

        let env = Env.ofList [ ("x", Boolean true) ]
        let stmt, env = Stmt.reduce stmt env
...
let ``reduce sequence`` =
    test "reduce sequence" {
        let stmt = Sequence(Assign("x", Add(Number 1, Number 1)), Assign("y", Add(Variable "x", Number 3)))
...
let ``reduce while`` =
    test "reduce while" {
        let stmt = While(LessThan(Variable "x", Number 5), Assign("x", Multiply(Variable "x", Number 3)))
...
let ``eval sequence`` =
    test "eval sequence" {
        ...
        Expect.equal expr (Number 5) ""
        let actual = Env.tryFind "x" env
        let expect = Some(Number 2)
        Expect.equal actual expect ""
        let actual = Env.tryFind "y" env
        let expect = Some(Number 5)
        Expect.equal actual expect ""
    }
...
let ``toRuby stmt`` =
    test "toRuby stmt" {
        let stmt = Assign("y", Add(Variable "x", Number 1))
